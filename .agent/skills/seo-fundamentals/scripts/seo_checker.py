...
def check_page(file_path: Path) -> dict:
    ...
    # 1. Title tag
    has_title = '<title' in content.lower() or 'title=' in content or 'Head>' in content or ('title' in content and has_metadata)
    if not has_title and (is_layout or has_metadata):
        issues.append("Missing <title> tag")
    
    # 2. Meta description
    has_description = 'name="description"' in content.lower() or 'name=\'description\'' in content.lower() or ('description' in content and has_metadata)
    if not has_description and (is_layout or has_metadata):
        issues.append("Missing meta description")
    
    # 3. Open Graph tags
    has_og = 'og:' in content or 'property="og:' in content.lower() or ('openGraph' in content and has_metadata)
    if not has_og and (is_layout or has_metadata):
